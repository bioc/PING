...
### Define the classes ##
#setClass("segReadsPE", representation(yF="numeric", yR="numeric", yFm="numeric", yRm="numeric", cF="numeric", 
#																			cR="numeric", cFm="numeric", cRm="numeric", map="matrix",chr="character"),
#					prototype(yF=numeric(0), yR=numeric(0), yFm=numeric(0), yRm=numeric(0), cF=numeric(0), cR=numeric(0), 
#										cFm=numeric(0), cRm=numeric(0), map=matrix(0,0,2),chr=character(0)))
#setClass("segReadsListPE", representation(List="list", paraSW="list", N="integer", NFm="integer", NRm="integer", 
#																					Nc="integer", NcFm="integer", NcRm="integer"), 
#					prototype(list(List=list(0), paraSW=list(islandDepth=integer(0),min_cut=integer(0),max_cut=integer(0)), 
#												 N=integer(0), NFm=integer(0), NRm=integer(0), Nc=integer(0), NcFm=integer(0), NcRm=integer(0))))
...
setClass("ping", representation(estimates="list",score="numeric",scoreF="numeric",scoreR="numeric",Nmerged="numeric",converge="logical",range="integer",chr="character"),prototype(estimates=list(w=numeric(0),mu=numeric(0),delta=numeric(0),sigmaSqF=numeric(0),sigmaSqR=numeric(0),seMu=numeric(0),seMuF=numeric(0),seMuR=numeric(0)),score=numeric(0),scoreF=numeric(0),scoreR=numeric(0),Nmerged=numeric(0),converge=logical(0),range=integer(0),chr=character(0)))

setClass("pingError", representation(errorCode="character"),prototype(errorCode=character(0)))

## Should I add some annotations?
setClass("pingList", representation(List="list", paraPrior="list", paraEM="list", minReads="list", N="integer", Nc="integer"), prototype(List=list(0),minReads=list(perPeak=integer(0),perRegion=integer(0)), paraPrior=list(xi=double(0),rho=double(0),alpha=double(0),beta=double(0)),paraEM=list(kMax=integer(0),B=integer(0),tol=double(0)),N=integer(0), Nc=integer(0)))
