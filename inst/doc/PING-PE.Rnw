...
Because Paired-End sequencing data often comes in the for of a .bam file, we provide a function to convert these files into \texttt{GRanges} with all the appropriate information.
